...
CANONICAL_JSON_OPTIONS = JSONOptions(
    datetime_representation=2, json_mode=1, document_class=dict, tz_aware=True)
...
def tasksManage(command, category, val=None, request=None):
    if command == 'get':
        ...
        return tasksManageAction(command, category, data)
...
def tasksManageAction(command, category, data):
    if command == 'add':
...
def tasksDataAdd(category, data):
    ...
    return taskDB.addDocumentToCollection("tasks", data)
    



def tasksDataUpdate(category, data):
    if category == 'task':
        data['updated'] = datetime.datetime.now()
