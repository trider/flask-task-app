...
def usersManage(command, category, val=None, request=None):
    if command == 'get':
        return usersDataGet(category, val)
    else:
        return 'Invalid command'
       
def usersDataGet(category, val=None, data=None, request=None):
    if category == 'users':
...
def tasksManage(command, category, val=None, request=None):
    if command == 'get':
        return tasksDataGet(category, val)
    else:
        return tasksManageAction(command, category, val, request)
...
def tasksManageAction(command, category, request):
    data = json.loads(request.data.decode('utf-8'))
    if command == 'add':
        return tasksDataAdd(category, data)
    elif command == 'update':
        return tasksDataUpdate(category, data)
    elif command == 'delete':
        return tasksDataDelete(category, data)


def tasksDataAdd(category, val=None, data=None):
    if category == 'task':
        data = json.loads(request.data)
        data['added'] = datetime.datetime.now()
        data['updated'] = datetime.datetime.now()
        taskDB.addDocumentToCollection('tasks', data)
        return 'Task added'
    else:
        return 'Invalid category'
    
def tasksDataUpdate(category, val=None, data=None):
    if category == 'task':
        data = json.loads(request.data)
        data['updated'] = datetime.datetime.now()
...
def tasksDataDelete(category, val=None, data=None):
    if category == 'task':
        taskDB.deleteItem('tasks', {'_id': ObjectId(data['_id'])})
        return 'Task deleted'
    else:
        return 'Invalid category'
